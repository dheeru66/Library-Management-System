...
export const deleteBook = async (req, res, next) => {
  ...
  try {
    const bookId = req.params.id; // Assuming the book ID is passed as a parameter in the URL
    ...
    res.status(200).json({ message: 'Book deleted successfully',allBooks });
  } catch (error) {
    next(error);
  }
};
...
export const updateBook = async (req, res, next) => {
  ...
  try {
    const { id } = req.params;
    const updatedBook = req.body; // Assuming req.body contains the updated book details

    // Find the book by ID and update it with the new details
